...
export function initBot(username) {
    const options = {
        ...
        auth: settings.auth,
        version: mc_version,
        checkTimeoutInterval: 120000,  // 120s keep-alive — Paper servers are slow
    }
    ...
    bot._client.emit = function(event, ...args) {
        if (event === 'error' && args[0]) {
            ...
            if (errStr.includes('PartialReadError') || errStr.includes('EPIPE') || errStr.includes('ECONNRESET') || errStr.includes('ended by the other party')) {
    ...
    bot.loadPlugin(toolPlugin); // auto equip best tool for block
